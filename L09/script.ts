...
class Mountain {
    ...
    constructor(peaks: { x: number, y: number }[]) {
        this.peaks = peaks;
    }
...
class Pond {
    ...
    getBounds(canvasWidth: number, canvasHeight: number) {
        return {
...
class Duck {
    ...
    constructor(x: number, y: number, speed: number, direction: number, pondBounds: { left: number, right: number, top: number, bottom: number }) {
        this.x = x;
        this.y = y;
        this.speed = speed;
...
class Bee {
    ...
    update(canvasWidth: number, canvasHeight: number) {
        ...
        if (this.x < 0 || this.x > canvasWidth) this.directionX *= -1;
        if (this.y < 0 || this.y > canvasHeight) this.directionY *= -1;
